  ...
  async function addContactName(transaction): Promise<object> {
    ...
    return Object.assign(transaction, {
      peerName: await getValue(
        "SELECT name AS value FROM contacts WHERE id = $1 AND userId = $2",
        [transaction.contactId, transaction.userId]
      )
    });
  }
...
function getData(user, resource): Promise<object[]> {
  const columns = {
    ...
    transactions:
      '"userId", "contact_id", "requested_at", "description", "direction", "amount"'
  };
  return runSql(
    `SELECT ${columns[resource]} FROM ${resource} WHERE userId = $1;`,
    [user.id]
  ).then(data => {
          ...
          try {
            ...
            if (req.method === "PUT") {
              ...
              if (resource === "contacts") {
                ...
                if (who === "new") {
                  contact.id = await getValue(
                    'INSERT INTO contacts ("userId", "name", "url", "token", "min", "max", "landmark") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id AS value;',
                    [
                      contact.userId,
                      contact.name,
                      contact.url,
                  ...
                  await runSql(
                    'UPDATE contacts SET "name" = $1, "url" =$2, "token" = $3, "min" = $4, "max" = $5, "landmark" = $6 WHERE userId = $7 AND id = $8;',
                    [
                      contact.name,
                      contact.url,
                      contact.token,
